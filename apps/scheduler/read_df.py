...
class csvtodf:
    ...
    def df_input(self):
        ...
        """
        index_Unnamed = [
            i for i, col in enumerate(self.df.columns) if "Unnamed: " in col
        ]
        df_kagisime_input = self.df.iloc[:, index_Unnamed[0] + 1 : index_Unnamed[1]]
        df_gomisute_input = self.df.iloc[
            :,
            index_Unnamed[1] + 1 : index_Unnamed[2] if len(index_Unnamed) > 2 else None,
        ]
        ...
        df_gomisute["必要人数"] = [
            4 if df_gomisute["参加可能人数"][i] > 4 else df_gomisute["参加可能人数"][i]
            for i in df_gomisute.index.values
        ]
        df_kagisime["必要人数"] = [
            2 if df_kagisime["参加可能人数"][i] > 2 else df_kagisime["参加可能人数"][i]
            for i in df_kagisime.index.values
        ]
    ...
    def convert(self, member, fname=None):
        ...
        for name, series in self.out_df.iteritems():
            ...
            for date, cell in zip(series_.index, series_):
                ...
                event.add(
                    "created", tokyo.localize(datetime.datetime.now())
                )  # いつ作ったのか．
